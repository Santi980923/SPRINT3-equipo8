...
def registrarUsuario(usuario,email,passwd,code):
    try:
        db=sqlite3.connect("mensajeria.s3db")
        db.row_factory=sqlite3.Row
        cursor=db.cursor()
        consulta="insert into usuarios (nombreusuario,correo,password,estado,codigoactivacion) values ('"+usuario+"','"+email+"','"+passwd+"','0','"+code+"')"
        cursor.execute(consulta)
        db.commit()
